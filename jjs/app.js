...
const boxCart = document.querySelector(".bx bx-cart-alt");
const openCart = document.querySelector(".cart__icon");
const closeCart = document.querySelector(".close__cart");
const overlay = document.querySelector(".cart__overlay");
const cartTotal = document.querySelector(".cart__total");
const clearCartBtn = document.querySelector(".clear__cart");
const sendMessageButton = document.querySelector(".sendMessage__cart");
...
class UI {
        ...
        <div class="product__footer">
          ...
          <div class="bottom">
            <div class="btn__group">
              <button class="btn addToCart" data-id= ${id} >Order</button>
            </div>
    ...
    getButtons() {
        ...
        buttons.forEach(button => {
            ...
            if (inCart) {
                button.innerText = "In Order";
                button.disabled = true;
            }
 
            button.addEventListener("click", e => {
                e.preventDefault();
                e.target.innerHTML = "In Order";
    ...
    addCartItem({ image, price, title, id }) {
        ...
        div.innerHTML = `<img src=${image} width="50" height="50" />
          <div>
            <h3>${title}</h3>
            <h3 class="price">Rp. ${price}</h3>
          </div>
          <div>
    ...
    cartLogic() {
        ...
        sendMessageButton.addEventListener("click", async() => {
            this.sendMessageCart();
            this.hide();
            try {
                let results = '';
                products.forEach((index, title, price, id) => {
                    results += `${index + 1}. ${title} \n   : ${id}    Harga: Rp.${toRupiah(price)} \n\n`;
                });
 
                const sendMessage = await liff.sendMessages([{
                    type: 'text',
                    text: `Hai ${profile.displayName}, \nTerimakasih telah oreder di JaJan Solo\n \nBerikut rincian Orderan anda : \n\n${results}Total : Rp.${toRupiah(itemTotal)} \n\nPesanan akan segera diproses bila sudah melakukan pembayaran.`,
                }, ]);
 
                cart = [];
                localStorage.clearCart('cart');
